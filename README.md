...
Atualmente, o projeto conta com autenticação via JWT para login e registro, também com criptografia de senhas, uma base de dados SQLite disponível no repositório com 30 séries para testes, e permite adicionar novas séries através da parte de linha de comando do aplicativo via API da OMDB.
...
## Atualizações
   ...
   *1.2 Adicionei um endpoint e para uma opção que mostra os 10 episódios mais bem avaliados de cada série na aba de detalhes para as mesmas.
   ...
   #1.3 Adicionei sistema completo de autenticação com Spring Security e JWT, incluindo criptografia das senhas e verificação de duplicidade dos nomes de usuário. 
...
## Recursos
    ...
    * Autenticação: completa via JWT, com criptografia de senhas.
...
## Tecnologias
    ...
    * Frontend: HTML, CSS, JavaScript para uma interface de usuário responsiva e interativa.
    * Backend: Java, Spring Boot e Spring security para gerenciar solicitações de API, fornecer dados e gerenciar logins e registros na base de dados.
...
A API backend, construída com Java e Spring Boot, oferece os seguintes endpoints:
    ...
    * Enviar usuário e senha para registro na Database: POST /register - Necessário enviar usuário e senha na head da solicitação.
    * Enviar usuário e senha já cadastrados para login: POST /login - Retorna o JWT Token necessário para acesso em todos os Endpoints.
...
## Detalhes do Backend
    ...
    * Controlador de autenticação (AuthController): Gerencia os endpoints para registro e login.
...
O backend interage com uma API externa para buscar dados sobre séries:
    ...
    * Para tradução das sinopses das séries
      URL Base: "https://api.mymemory.translated.net/get?q=" + texto + "&langpair=" + langpair";
    

## Tratamento de Erros: 
    Caso a chave da API esteja incorreta ou a API esteja fora do ar, você receberá um erro 401 ou 500. Verifique a configuração da chave e a disponibilidade da API.
...
## Como Utilizar
    ...
    Navegue até o diretório do projeto e execute o arquivo TvDbApplication.java para iniciar o servidor.
    Abra o arquivo login.html com um servidor ao vivo, como o Live Server no Visual Studio Code.
...
Currently, the project includes JWT-based authentication for login and registration, password encryption, an SQLite database available in the repository with 30 series for testing, and the ability to add new series via the command line application through the OMDB API.
...
## Updates:
    ...
    * 1.3: Implemented a complete authentication system with Spring Security and JWT, including password encryption and username duplication checking.

## Features

    * Homepage: Sections for new releases, popular titles, and a carousel for various TV series.
    * Authentication: Uses JWT authentication to register and login.
...
The backend API, built with Java and Spring Boot, provides the following endpoints:
    ...
    * Register User: POST /register - Requires sending username and password in the request header.
    * Login: POST /login - Returns the JWT Token required for accessing all endpoints.
...
Backend Details
    ...
    * Authentication controller (AuthController): Handles register and login endpoints.
...
## How to Use
    ...
    * Open the login.html file with a live server, such as the Live Server in Visual Studio Code.
